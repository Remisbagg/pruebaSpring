...
public interface IFriendshipRepository extends JpaRepository<Friendship, Long> {
    ...
    @Query("select new com.back.dto.response.friendship.FriendDTO(f.id, f.user2.id, f.user2.avatar, f.user2.firstName, f.user2.lastName," +
    ...
    @Query("select new com.back.dto.response.friendship.FriendDTO(f.id, f.user1.id, f.user1.avatar, f.user1.firstName, f.user1.lastName," +
